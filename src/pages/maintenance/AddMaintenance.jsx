...
// Searchable Dropdown (Compact height applied)
const SearchableAssetSelect = ({
  ...
  const selectedAsset = assets.find(
    (a) =>
      String(a.AssetID || a.assetID || a.assetId || a.id) === String(value),
  );
  ...
  return (
    <div className="relative" ref={dropdownRef}>
      <div
        className="w-full flex items-center justify-between rounded-lg border border-slate-300 bg-white px-2.5 py-1.5 text-xs outline-none focus-within:border-blue-500 focus-within:ring-2 focus-within:ring-blue-100 cursor-text"
      ...
      >
        <input
          ...
          placeholder={
            displayValue || placeholder || "Search asset name or tag..."
          }
        ...
        <div className="absolute z-[999] mt-1 max-h-60 w-full overflow-auto rounded-md bg-white py-1 shadow-lg ring-1 ring-black ring-opacity-5 border border-slate-200">
          {filtered.length === 0 ? (
            ...
            filtered.map((asset, i) => {
              ...
              const astName =
                asset.AssetName || asset.assetName || `Asset ${i + 1}`;
              const astTag = asset.AssetTag || asset.assetTag || "";
              return (
                <div
                  key={astId}
                  className={`cursor-pointer px-3 py-1.5 text-xs hover:bg-blue-50 hover:text-blue-700 ${String(astId) === String(value) ? "bg-blue-50 text-blue-700 font-medium" : "text-slate-700"}`}
                  onClick={() => {
                    onChange(astId);
                    setSearch("");
                    setIsOpen(false);
...
export const AddMaintenance = () => {
  ...
  const getAssetName = (record) => {
    const recordId = String(
      record.AssetID || record.assetId || record.assetID || "",
    );
    const assetMatch = assets.find(
      (a) => String(a.AssetID || a.assetId || a.assetID || a.id) === recordId,
    );
    return (
      record.AssetName ||
      record.assetName ||
  ...
  return (
    <div className="min-h-screen bg-slate-50 p-3 sm:p-5">
      <div className="mx-auto max-w-[1400px] space-y-4">
        {/* Maintenance Tracker Header with -mt-4 to reduce top gap */}
        {/* Maintenance Tracker Header - Upar shift karne ke liye aur chota karne ke liye */}
        ...
        {error && (
          <div className="flex items-start justify-between gap-3 rounded-lg border border-red-200 bg-red-50 px-4 py-2.5 text-xs text-red-700">
            <span>{error}</span>
            <button
              type="button"
              onClick={() => setError("")}
        ...
        <div className="grid grid-cols-1 items-start gap-4 xl:grid-cols-[360px_minmax(0,1fr)]">
          {/* Form Section */}
          <section className="relative z-20 rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
            ...
            <form onSubmit={handleSubmit} className="space-y-3">
              ...
              <div>
                ...
                <input
                  ...
                  required
                  className="w-full rounded-lg border border-slate-300 px-2.5 py-1.5 text-xs outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-100"
                />
              </div>
              ...
              <div>
                ...
                <textarea
                  ...
                  onChange={handleChange}
                  placeholder="Details..."
                  className="w-full resize-y rounded-lg border border-slate-300 px-2.5 py-1.5 text-xs outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-100"
                />
              </div>
              ...
              <div>
                ...
                <input
                  ...
                  onChange={handleChange}
                  className="block w-full rounded-lg border border-dashed border-slate-300 p-1.5 text-xs text-slate-600 file:mr-2 file:rounded-md file:border-0 file:bg-blue-50 file:px-2.5 file:py-1 file:text-xs file:font-medium file:text-blue-700"
                />
              </div>
              ...
              <button
                type="submit"
                disabled={saving}
                className="flex w-full items-center justify-center gap-2 rounded-lg bg-blue-700 px-3.5 py-2 text-xs font-semibold text-white transition hover:bg-blue-800 disabled:cursor-not-allowed disabled:opacity-60"
              >
                <Plus size={15} />
                {saving
                  ? "Saving..."
          ...
          <div className="flex flex-col gap-4 min-w-0">
            ...
            <section className="relative z-10 rounded-xl border border-slate-200 bg-white shadow-sm">
              <div className="border-b border-slate-100 bg-slate-50/50 px-4 py-3 rounded-t-xl">
                <h2 className="flex items-center gap-2 text-sm font-semibold text-slate-800">
              ...
              <div className="p-4 flex flex-col sm:flex-row sm:items-start justify-between gap-4">
                ...
                {selectedInsight && selectedInsight.totalRepairs > 0 ? (
                  <div className="flex flex-wrap gap-3 flex-1">
                    <div className="flex-1 flex flex-col items-center justify-center rounded-xl bg-orange-50 border border-orange-100 p-3 text-center shadow-sm">
                      <WrenchIcon size={18} className="mb-1 text-orange-600" />
                      <p className="text-[11px] font-semibold text-orange-600 uppercase">
                        Total Repairs
                      </p>
                      <p className="mt-0.5 text-xl font-bold text-orange-900">
                        {selectedInsight.totalRepairs}
                      </p>
                    </div>
                    <div className="flex-1 flex flex-col items-center justify-center rounded-xl bg-emerald-50 border border-emerald-100 p-3 text-center shadow-sm">
                      <IndianRupee
                        size={18}
                      ...
                      <p className="mt-0.5 text-xl font-bold text-emerald-900">
                        ₹
                        {Number(selectedInsight.totalCost).toLocaleString(
                          "en-IN",
                        )}
                      </p>
                    </div>
                  ...
                  <div className="flex-1 flex items-center justify-center rounded-xl border border-dashed border-slate-200 bg-slate-50 p-4 text-xs text-slate-500">
              ...
              <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-100 px-4 py-3">
                ...
                <div className="relative w-full sm:w-64">
                  ...
                  <input
                    ...
                    placeholder="Search asset name or tag..."
                    className="w-full rounded-lg border border-slate-300 py-1.5 pl-8 pr-3 text-xs outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-100"
                  />
                </div>
              </div>
                      ...
                      <tbody className="divide-y divide-slate-100">
                        {paginatedRecords.length === 0 ? (
                          ...
                          paginatedRecords.map((record, index) => {
                            ...
                            return (
                              <tr
                                key={record.MaintenanceID ?? index}
                                className="hover:bg-slate-50"
                              >
                                <td className="whitespace-nowrap px-3.5 py-2.5 text-slate-500">
                                  {(currentPage - 1) * pageSize + index + 1}
                                </td>
                                <td className="whitespace-nowrap px-3.5 py-2.5 font-medium text-blue-700">
                                  {getAssetTag(record)}
                                </td>
                                ...
                                <td
                                  className="max-w-[180px] truncate px-3.5 py-2.5 text-slate-600"
                                  title={
                                ...
                                <td className="px-3.5 py-2.5">
                                  {receiptName ? (
                                    <a
                                      href={`http://192.168.1.17:5000/${record.ReceiptFilePath || record.receiptFilePath}`}
                                      target="_blank"
                                      rel="noreferrer"
                                      className="inline-flex max-w-[130px] items-center gap-1 truncate text-xs text-blue-700 hover:underline"
                                      title={receiptName}
                                    >
                                      <FileText size={14} />
                                      {receiptName}
